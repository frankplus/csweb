...
void GameStateOptions::Create()
{
	...
	mControlsTex = JResourceManager::LoadTextureFromFile("gfx/controls.png");
...
void GameStateOptions::Update(float dt)
{
	if (!gDanzeff->mIsActive) {
		if (mEngine->GetButtonClick(CTRL_LTRIGGER)) {
			mState--;
			if (mState < 0) {
				mState = CONTROLS;
			}
		}
		if (mEngine->GetButtonClick(CTRL_RTRIGGER)) {
			mState++;
			if (mState > CONTROLS) {
				mState = 0;
			}
		}
		if (mEngine->GetButtonState(CTRL_RTRIGGER) || mEngine->GetButtonState(CTRL_LTRIGGER))  {
			for (int i=0; i<NUMCONFIGS; i++) {
