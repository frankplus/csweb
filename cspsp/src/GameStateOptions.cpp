...
void GameStateOptions::Render()
{
	...
	if (mState == OPTIONS) {
		...
		mRenderer->FillRect(0,35,SCREEN_WIDTH_F,175,ARGB(100,0,0,0));
		...
		if (main) {
			mRenderer->FillRect(0,35+index*25,SCREEN_WIDTH_F,25,ARGB(255,0,0,0));
			gFont->DrawShadowedString("[X] Select     [O] Cancel and Return to Menu",SCREEN_WIDTH_2,SCREEN_HEIGHT_F-20,JGETEXT_CENTER);
		}
		else {
			mRenderer->FillRect(0,35+index*25,SCREEN_WIDTH_F,25,ARGB(100,0,128,255));
			if (!gDanzeff->mIsActive) {
