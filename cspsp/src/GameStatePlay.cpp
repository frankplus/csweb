...
void GameStatePlay::CheckInput(float dt)
{
	...
	if(deltaMouseX != 0 || deltaMouseY != 0)
		mPlayer->MoveCursor(deltaMouseX, deltaMouseY);
...
void GameStatePlay::Update(float dt)
{
	...
	/**
	if (mEngine->GetButtonClick(CTRL_TRIANGLE))	// do a screen shot when TRIANGLE is pressed
	{
		char s[80];
		sprintf(s, "screenshot.png");
		mEngine->ScreenShot(s);
	}**/
	if (mEngine->GetButtonClick(CTRL_NOTE))
	{
		//ResetRound();
